...
export const exercisesMapper = createSelector(
  ...
  (exercises) => {
    if (Object.keys(exercises).length !== 0) {
      return Object.keys(exercises).reduce((acc, x) => {
        const repsWeight = `${exercises[x].reps} ${exercises[x].weight}`;
        const sets = acc[exercises[x].type][repsWeight] ? acc[exercises[x].type][repsWeight] += 1 : 1;
        return {
          ...acc,
          [exercises[x].type]: {
...
export const exercisesSelector = createSelector(
  exercisesMapper,
  exercises => exercises && Object.keys(exercises).map(type => ({
